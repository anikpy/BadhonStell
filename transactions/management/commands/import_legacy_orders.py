...
from shop.models import (
    Customer as LegacyCustomer, Order, OrderItem,
)
from transactions.models import (
    Customer, Transaction, TransactionItem, TransactionHistory
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if customer_id:
            ...
            # Import all customers from legacy system
            customers = LegacyCustomer.objects.all()
    ...
    def import_customer(self, customer, dry_run=False):
        ...
        # Check if customer already exists in transaction system
        ...
        if dry_run:
            ...
            self.stdout.write(f'    Would create: 1 customer + {order_count} purchase transactions')
            return True
        ...
        self.stdout.write(f'    ✓ Created customer (ID: {new_customer.pk})')
        ...
        for order in orders:
            try:
                self.import_order_as_purchase(order, new_customer)
                imported_orders += 1
                
                # If order has cash_paid, create a submission transaction
                if order.cash_paid and order.cash_paid > 0:
                    self.import_order_payment_as_submission(order, new_customer)
            except Exception as e:
                self.stdout.write(
    ...
    def import_order_as_purchase(self, order, new_customer):
        """Import a custom order as a purchase transaction"""
        # Get all order items
    ...
    def import_order_payment_as_submission(self, order, new_customer):
        ...
        transaction = Transaction.objects.create(
            ...
            item_name=f'Order #{order.pk} Payment',
